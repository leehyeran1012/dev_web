    ...
    function toggleChecktbox(id, id2) {
        var val3 = "";
        var chkok = "";

        if($("input[name=wrb"+id+"_"+id2+"]").is(":checked")) {
            $("input[name=wrd"+id+"_"+id2+"]").each(function() {
                $(this).prop('disabled', false);    //활성
            });

			$(".lbl"+id+"_"+id2).text(function(i, text) {
				return text.replaceAll("☆", "★");
			});

/*
           $("input[name=wrd"+id+"_"+id2+"]").each(function(idx) {
                chkok = $(this).val();
           });
           if(chkok==1) {
               alert("별점에 한개 체크해 주세요.");
           }
*/
            ...
            $("input[name=wrd"+id+"_"+id2+"]").each(function() {
                ...
                $("input[name=wrd"+id+"_"+id2+"]").each(function() {
                    $("input[name=wrd"+id+"_"+id2).prop("checked", false);
                });
...
function jusointput() {
	$("#wr_addr").val($("#wr_postcode").val()+"|"+$("#juso1").val()+"|, "+$("#juso2").val()+"|"+$("#juso3").val());
}