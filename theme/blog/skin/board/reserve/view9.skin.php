...
$today = date("Y-m-d");
$result = sql_query("select wr_id,wr_subject,wr_content,wr_name,wr_last,wr_1,wr_2,wr_3,wr_4,wr_5,wr_6,wr_7,wr_8,wr_9,wr_10 from {$boa_table} where left(wr_1,7) = '{$t_date}' order by wr_name");
...
<div class="container-fluid mb-5">
		...
		<div class="row border-bottom pt-4 pb-2 mb-5">
		  <div class="col-sm-12 col-md-6 fs-4 fw-bold"><i class="bi bi-file-medical-fill me-2"></i>신청결과보기(<?= substr($que1["wr_subject"],0,7) ?>)</div>
		  <div class="col-sm-12 col-md-6 text-end"><a class="btn btn-outline-success btn-sm me-3" href="<?= $board_skin_url ?>/excel_view.php?bo_table=<?=$bo_table?>&g_num=<?=$que1["wr_id"]?>">엑셀로저장</a><a class="btn btn-outline-secondary btn-sm" href="<?= get_pretty_url($bo_table); ?>">목록으로</a></div>
		</div>
		...
		<div class="table-responsive">
		 <table class="table mb-4 align-middle table-hover text-center" style="min-width:1200px">
			...
			<tr>
				...
				<th style="width: 6rem;">참여자</th>
				<th style="width: 6rem;">예약일자</th>
				<th style="width: 6rem;">예약시간</th>
		...
		<?php foreach($result as $field) {
			...
			<?php } ?>
			<?php if (count($result) == 0) { echo '<tr><td colspan="2" class="empty_table">게시물이 없습니다.</td></tr>'; } ?>
			</tbody>
		</table>
	</div>
</div>
