...
<?php
	...
	foreach($subtitles as $key=>$value) {
		...
		if (mb_strpos($sub_title, "메일") !== false) $mailok = 1;
	...
	<?php } elseif (substr($value2,0,1) == "s") {
		if($mailok == 1) {	?>
				<div class="input-group">
				  <input type="text" class="form-control" name="email" size="15" id="email" value="">
				  <span class="input-group-text">@</span>
				  <input type="text" class="form-control" name="email2" size="15" id="email2" value="">
					<select class="form-select" id="email_sel">
						<option class="person_sel" value="">직접입력</option>
				...
				<input type="hidden" id="wrk_mail" name="wr_<?=$xx?>" value="<?= trim($value2) ?>">
				<script>
					$(function() {
						$("#email_sel").change(function(){
							var email_text = $(this).children("option:selected").text();
							var email_direct = $(this).children("option:first-of-type").text();
							if (email_text == email_direct){
								$(this).siblings("#email2").val("");
							} else {
								$(this).siblings("#email2").val(email_text);
							}

							var input1 = document.getElementById('email').value;
							var input2 = document.getElementById('email2').value;
						 document.getElementById('wrk_mail').value = input1+"@"+input2;
