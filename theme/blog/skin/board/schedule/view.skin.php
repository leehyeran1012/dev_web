...
<div class="container">
	...
	<h3><?= cut_str(get_text($view['wr_subject']), 70); ?></h3><hr>
		...
		<div>
			<ul class="list-inline mb-0">
				<li class="list-inline-item">
					<a href="#" data-bs-toggle="dropdown" class="text-dark"><?= get_text($view['wr_name']); ?></a>
					<?php if ($is_ip_view) { ?>
	...
	<div class="d-flex flex-sm-row flex-column justify-content-sm-between mb-4">
		<div class="d-flex justify-content-center mb-2 mb-sm-0">
			...
			<div class="btn-group xs-100">
				<?php if ($update_href) { ?><a href="<?= $update_href ?>" class="btn btn-danger"><i class="fa fa-pencil-square-o"></i> 수정</a><?php } ?>
				<?php if ($delete_href) { ?><a href="<?= $delete_href ?>" onclick="del(this.href); return false;" class="btn btn-danger"><i class="fa fa-trash-o"></i> 삭제</a><?php } ?>
				<?php if ($copy_href) { ?><a href="<?= $copy_href ?>" onclick="board_move(this.href); return false;" class="btn btn-danger"><i class="fa fa-copy"></i> 복사</a><?php } ?>
				<?php if ($move_href) { ?><a href="<?= $move_href ?>" onclick="board_move(this.href); return false;" class="btn btn-danger"><i class="fa fa-arrows-alt"></i> 이동</a><?php } ?>
				<?php if ($search_href) { ?><a href="<?= $search_href ?>" class="btn btn-danger"><i class="fa fa-search"></i> 검색</a><?php } ?>
			</div>
			<?php } ?>
		</div>
		<div class="d-flex justify-content-center">
			<div class="btn-group xs-100">
				<a href="<?= $list_href ?>" class="btn btn-primary"><i class="fa fa-list"></i> 목록</a>
				<?php if ($scrap_href) { ?><a href="<?= $scrap_href;  ?>" target="_blank" class="btn btn-primary" onclick="win_scrap(this.href); return false;"><i class="fa fa-clipboard"></i> 스크랩</a><?php } ?>
				<?php if ($reply_href) { ?><a href="<?= $reply_href ?>" class="btn btn-primary"><i class="fa fa-reply"></i> 답변</a><?php } ?>
				<?php if ($write_href) { ?><a href="<?= $write_href ?>" class="btn btn-primary"><i class="fa fa-pencil"></i> 글쓰기</a><?php } ?>
			</div>
		</div>
	</div>
	...
	<ul class="list-group mb-4">
		<?php if ($prev_href) { ?><li class="list-group-item"><small class="text-muted"><i class="fa fa-caret-up"></i><span class="d-none d-md-inline"> 이전글</span></small> <a href="<?= $prev_href ?>" class="text-dark"><?= $prev_wr_subject;?></a> <small class="float-end text-muted d-none d-md-inline"><?= str_replace('-', '.', substr($prev_wr_date, '2', '8')); ?></small></li><?php } ?>
		<?php if ($next_href) { ?><li class="list-group-item"><small class="text-muted"><i class="fa fa-caret-down"></i><span class="d-none d-md-inline"> 다음글</span></small> <a href="<?= $next_href ?>" class="text-dark"><?= $next_wr_subject;?></a> <small class="float-end text-muted d-none d-md-inline"><?= str_replace('-', '.', substr($next_wr_date, '2', '8')); ?></small></li><?php } ?>
	</ul>
	<?php } ?>
