	...
	if($accessCode == 1) {
		foreach($result as $field) {
			if(strlen($field["wr_1"]) > 11 && substr($field["wr_2"],0,10) == $to_day) {
				$cont = $cont."<p class='text-dark'>☆ (".$field["wr_name"].") ".substr($field["wr_1"],11,5)." ".$field['wr_subject']."</p>";
			} elseif(strlen($field["wr_1"]) == 10 && substr($field["wr_2"],0,10) > $to_day) {
				$cont = $cont."<p class='text-dark'>☆ (".$field["wr_name"].") ".$field['wr_subject']."</p>";
			}
		}
		...
		foreach($result as $field) {
			if(strlen($field["wr_1"]) > 11 && substr($field["wr_2"],0,10) == $to_day) {
				$cont = $cont."<p class='text-dark'>☆ ".substr($field["wr_1"],11,5)." ".$field['wr_subject']."</p>";
			} elseif(strlen($field["wr_1"]) == 10 && substr($field["wr_2"],0,10) > $to_day) {
				$cont = $cont."<p class='text-dark'>☆ ".$field['wr_subject']."</p>";
			}
		}
...
if($cont == "") {
	echo "<p class='text-dark'>● 오늘 일정이 없습니다.</p>";
} else {
	echo $cont;
}
