...
// 페인지 아이디 생성
sql_query(" ALTER TABLE `{$g5['menu_table']}` ADD `me_pid` varchar(255) NOT NULL DEFAULT '' AFTER `me_code` ", false);
...
.td_numxlg { width:130px;text-align:center }
</style>
...
<div id="menulist" class="tbl_head01 tbl_wrap">
    ...
    ?>
    <tr class="<?php echo $bg; ?> menu_list menu_group_<?php echo (strlen($row['me_code']) == 4)?substr($row['me_code'], 0, 4):substr($row['me_code'], 0, 2).$sub_class;?>">
        <td class="td_mngsmall">
			<?php echo (strlen($row['me_code']) == 2) ? $row['me_code'] : '&nbsp;'?>
		</td>
		<td class="td_numxlg">
            <label for="me_pid_<?php echo $i; ?>" class="sound_only">pid</label>
            <input type="text" name="me_pid[]" value="<?php echo $row['me_pid'] ?>" id="me_pid_<?php echo $i; ?>" class="tbl_input full_input">
        </td>
		<td class="td_category<?php echo $sub_menu_class; ?>">
            <input type="hidden" name="code[]" value="<?php echo (strlen($row['me_code']) == 4) ? substr($row['me_code'], 0, 4) : substr($row['me_code'], 0, 2); ?>">
        ...
        <td class="td_mng">
			...
			<?php } else if(strlen($row['me_code']) == 4) { ?>
	            <button type="button" class="btn_add_submenu2 btn_01">추가</button>
