...
$(function() {
    ...
    $(document).on("click", "#add_manual", function() {
		...
		var me_pid = $.trim($("#me_pid").val());

        add_menu_list(me_name, me_link, me_pid, "<?php echo $code; ?>");
    });
    ...
    $(document).on("click", ".add_select", function() {
		...
		var me_pid = $.trim($(this).siblings("input[name='pid[]']").val());

        add_menu_list(me_name, me_link, me_pid, "<?php echo $code; ?>");
    });
});

function add_menu_list(name, link, pid, code)
{
	...
	list += "<td class=\"td_numxlg\">";
    list += "<label for=\"me_pid_"+ms+"\"  class=\"sound_only\">pid</label>";
    list += "<input type=\"text\" name=\"me_pid[]\" value=\""+pid+"\" id=\"me_pid_"+ms+"\" class=\"frm_input full_input\">";
    list += "</td>";
	...
	<?php } else if($sub) { ?>
    list += "<button type=\"button\" class=\"btn_add_submenu2 btn_01\">추가</button>\n";
